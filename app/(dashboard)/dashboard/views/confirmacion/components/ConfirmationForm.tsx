...
export const ConfirmationForm:FC<FormProps> = ({hSubmit, oSubmit, reg, err, reseter}) => {
  
  const [activeNote, setActiveNote] = useState<boolean>(false)
  
  return (
    <form onSubmit={hSubmit(oSubmit)}> 
                <Grid2 container direction='column' justifyContent='center' alignItems='center' height='400px' sx={{overflowY: 'scroll'}}>
                  ...
                  <Grid2 container direction='row' border='1px solid grey' borderRadius={5} p={1} mt={2} boxShadow={2}>
                    <Stack>
                      ...
                      <FormControl sx={{mt:2, position:'relative'}} error={!!err.fechaConfirmacion}>
                        <FormLabel>Fecha de Confirmacion *</FormLabel>
                        <input type="date" placeholder="fechaConfirmacion" {...reg("fechaConfirmacion", {required: true})} />
                        {err.fechaConfirmacion &&
                                    <FormHelperText>
                                      <InfoOutlined />
                                      Requerido
                                    </FormHelperText>}
                      ...
                      <FormControl sx={{mt:2, position:'relative'}} error={!!err.edadConfirmado}>
                         <FormLabel>Edad del Confirmado *</FormLabel>
                         <Input
                          sx={{top: -15, width: '50px'}}
                          type="text"
                          {...reg("edadConfirmado", {required: true})}
                      />
                      {err.edadConfirmado &&
                                    <FormHelperText>
                                      <InfoOutlined />
                    ...
                    <FormControl sx={{mt:2, position:'relative', mx: 2}} error={!!err.confirmadoNombre}>
                      <FormLabel>Nombre del Confirmado (completo)*</FormLabel>
                      <Input
                          sx={{top: -15}}
                          type="text"
                          {...reg("confirmadoNombre", {required: true})}
                      />
                      {err.confirmadoNombre &&
                                  <FormHelperText>
                                    <InfoOutlined />
                                    Requerido
                                  </FormHelperText>}
